...
function LogoItem({ brand }: { brand: typeof brands[0] }) {
  ...
  // Size classes: normal = 32px, large = 40px, xlarge = 56px, xxlarge = 64px
  const sizeClass = brand.size === "xxlarge" 
    ? "max-h-16" 
    : brand.size === "xlarge" 
      ? "max-h-14" 
      : brand.size === "large" 
  ...
  // Only show original colors on hover for specific brands
  const showOriginalColors = isHovered && brand.hasColorHover
    ...
    >
      <Image
        ...
        className={`
          ...
          ${brand.needsBlendMode && !showOriginalColors ? "mix-blend-screen" : ""}
        `}
        style={{
          filter: showOriginalColors ? "none" : "brightness(0) invert(1)"
        }}
      />
    </div>
  )
