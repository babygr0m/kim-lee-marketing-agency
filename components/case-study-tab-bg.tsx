...
const GRADIENTS: GradientDef[] = [
  // Fashion Nova — radial pink-to-deep-red drift. Hot pink core bleeds
  // into deep red before fading to the dark page base. Opacities lifted
  // (40% / 28%) so the brand presence reads clearly across the viewport
  // without compromising text legibility.
  {
    ...
    style: {
      backgroundImage:
        "radial-gradient(circle at center, rgba(255, 45, 122, 0.40) 0%, rgba(139, 0, 48, 0.28) 30%, #0A0A0A 70%)",
      backgroundSize: "200% 200%",
    },
  },
  // Forever 21 — wide yellow band sweeping diagonally over black. Band
  // pushed to 55% opacity so the signature F21 yellow is unmistakably
  // visible as it sweeps; widened from 30–70% range to 25–75% for a
  // broader, more present band.
  {
    slug: "forever-21",
    animateClass: "animate-cs-forever-21",
    style: {
      backgroundImage:
        "linear-gradient(135deg, #0A0A0A 0%, #0A0A0A 25%, rgba(255, 229, 0, 0.55) 50%, #0A0A0A 75%, #0A0A0A 100%)",
      backgroundSize: "300% 300%",
    },
  },
  // Ivy City Co — champagne-cream radial drift. Warm cream center bleeds
  // through brand gold and out to black. Lifted to 45% / 30% so the
  // signature gold-on-black wedding aesthetic carries actual brand weight.
  {
    slug: "ivy-city",
    animateClass: "animate-cs-ivy-city",
    style: {
      backgroundImage:
        "radial-gradient(circle at center, rgba(244, 228, 193, 0.45) 0%, rgba(201, 169, 97, 0.30) 30%, #0A0A0A 70%)",
      backgroundSize: "200% 200%",
    },
  },
  // Sol de Janeiro — tropical orange→coral→green diagonal sweep at 110°.
  // All three stops bumped roughly 2× so the Brazilian summer palette
  // actually pops alongside the three boosted gradients above.
  {
    slug: "sol-de-janeiro",
    animateClass: "animate-cs-sol-de-janeiro",
    style: {
      backgroundImage:
        "linear-gradient(110deg, rgba(255, 107, 53, 0.38) 0%, rgba(255, 154, 139, 0.25) 50%, rgba(45, 155, 79, 0.32) 100%)",
      backgroundSize: "200% 200%",
    },
  },
]
...
export function CaseStudyTabBg({ activeSlug }: CaseStudyTabBgProps) {
  return (
    <div
      ...
      // area, regardless of scroll position. NEGATIVE z-index (-z-10) sits
      // unambiguously beneath every content element in normal flow (which
      // is z-auto), so the page hero, tab nav, active card content, and
      // closing CTA all stack above the gradient without needing their
      // own explicit positive z-index. The fixed Header (`z-50`) still
